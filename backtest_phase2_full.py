"""
Phase 2 Full Backtest — 3 coins × 3 days
Validates false positive rate for all detectors.

Uses actual 1h taker net data from CoinGlass + price from DB.
Evaluates: did price move in signaled direction within 4 hours?
"""

# ══════════════════════════════════════════════════════════════════════
# RAW DATA — CoinGlass 1h taker net + price (72h lookback)
# Format: (date, hour_wib, taker_net_usd, price_close)
# Pulled from CoinGlass pair_taker + coinglass_price_ohlc
# ══════════════════════════════════════════════════════════════════════

# BTC — 72h taker net 1h candles (Apr 5 12:00 → Apr 8 11:00 WIB)
# From CoinGlass API + investigation report + DB price data
BTC_DATA = [
    # Apr 6 (sideways day — good for false positive testing)
    ...
    # Apr 8 — pump continuation (from CoinGlass confirmed data)
...
# SOL — from investigation report + CoinGlass confirmed
SOL_DATA = [
    # Apr 6 (sideways)
    ...
    # Apr 7 — dump into pump (from report)
    ...
    # Apr 8 — pump (from CoinGlass + report)
...
# ETH — from CoinGlass confirmed + DB price
ETH_DATA = [
    # Apr 6 (sideways)
    ...
    # Apr 8 — pump (from CoinGlass confirmed)
...
FUTCVD_TOLERANCE = {
    "BTC": 5_000_000, "ETH": 2_000_000, "SOL": 500_000,
}
CLIMACTIC_THRESH = {
    "BTC": 50_000_000, "ETH": 20_000_000, "SOL": 5_000_000,
}
DIVERGENCE_MIN = {
    "BTC": 50_000_000, "ETH": 20_000_000, "SOL": 5_000_000,
}
...
def simulate_all(coin, data):
    """Simulate all 3 detectors on 1h taker data. Return list of triggers."""
    triggers = []
    ...
    taker_history = []
    # For divergence: track consecutive deltas
    ...
    for i, (day, hour, taker_net, price) in enumerate(data):
        ...
        # Use 6h window (6 candles at 1h)
        window = taker_history[-6:] if len(taker_history) >= 6 else taker_history
        max_sell = min(window)
        max_buy = max(window)

        # FutCVD delta proxy = current taker_net (simplified: in real system this is separate)
        # For divergence we need consecutive deltas
        fut_delta = taker_net  # proxy

        # Price 4h later for evaluation
        price_4h = data[min(i + 4, len(data) - 1)][3]

        # ── TIER 1A: EXHAUSTION ──
        if max_sell < 0 and (i - last_cooldown["1A"]) >= 6:
            if taker_net < 0:
            ...
            if ratio < EXHAUSTION_RATIO:
                if fut_delta >= -tol:
                    ...
                    triggers.append({
                        "detector": "1A_EXHAUSTION",
                        "time": idx_label,
                        "price": price,
                        "direction": direction,
                        "price_4h": price_4h,
                        "correct": correct,
                        "detail": f"ratio={ratio:.1%} taker={taker_net:+,.0f} max={max_sell:+,.0f}",
                    })
                    last_cooldown["1A"] = i

        if max_buy > 0 and (i - last_cooldown["1A"]) >= 6:
            if taker_net > 0:
            ...
            if ratio < EXHAUSTION_RATIO:
                if fut_delta <= tol:
                    ...
                    triggers.append({
                        ...
                        "price_4h": price_4h,
                        "correct": correct,
                        "detail": f"ratio={ratio:.1%} taker={taker_net:+,.0f} max={max_buy:+,.0f}",
                    })
                    last_cooldown["1A"] = i
        ...
        if (i - last_cooldown["1B"]) >= 6:
            ...
            if taker_net == max_buy and taker_net > ct:
                if fut_delta <= abs(taker_net) * 1.5:
                    direction = "SHORT"
                    correct = price_4h < price
                    triggers.append({
                        ...
                        "price_4h": price_4h,
                        "correct": correct,
                        "detail": f"taker={taker_net:+,.0f} (MAX BUY)",
                    })
                    last_cooldown["1B"] = i

        # ── TIER 2: DIVERGENCE (simplified with consecutive positive/negative) ──
        if i >= 2 and (i - last_cooldown["2"]) >= 6:
            d1 = data[i-1][2]
            d0 = data[i][2]
            if d1 > 0 and d0 > 0:  # 2 consecutive positive
                total = d1 + d0
                if abs(total) >= dm:
                    direction = "LONG"
                    correct = price_4h > price
                    triggers.append({
                        "detector": "2_DIVERGENCE",
                        "time": idx_label,
                        "price": price,
                        "direction": direction,
                        "price_4h": price_4h,
                        "correct": correct,
                        "detail": f"streak=2 total={total:+,.0f}",
                    })
                    last_cooldown["2"] = i
            elif d1 < 0 and d0 < 0:  # 2 consecutive negative
                total = d1 + d0
                if abs(total) >= dm:
                    direction = "SHORT"
                    correct = price_4h < price
...
print("=" * 90)
print("PHASE 2 FULL BACKTEST — 3 coins × 3 days (Apr 6-8)")
print("Evaluation: price direction correct within 4 hours?")
print("=" * 90)
...
for coin, data in [("BTC", BTC_DATA), ("ETH", ETH_DATA), ("SOL", SOL_DATA)]:
    ...
    print(f"\n{'─' * 90}")
    print(f"  {coin} — {len(triggers)} triggers total")
    print(f"{'─' * 90}")
...
print("\n" + "=" * 90)
print("FALSE POSITIVE RATE PER DETECTOR PER COIN")
print("=" * 90)

detectors = ["1A_EXHAUSTION", "1B_CLIMACTIC", "2_DIVERGENCE"]

# Header
print(f"\n  {'Detector':<20} | {'Coin':<5} | {'Triggers':<8} | {'Correct':<8} | {'Wrong':<6} | {'Accuracy':<10} | {'FP Rate':<10} | {'Status'}")
print(f"  {'─'*20} | {'─'*5} | {'─'*8} | {'─'*8} | {'─'*6} | {'─'*10} | {'─'*10} | {'─'*10}")

for det in detectors:
    for coin in ["BTC", "ETH", "SOL"]:
        coin_det = [t for t in all_results[coin] if t["detector"] == det]
        total = len(coin_det)
        correct = sum(1 for t in coin_det if t["correct"])
        wrong = total - correct
        accuracy = correct / total * 100 if total > 0 else 0
        fp_rate = wrong / total * 100 if total > 0 else 0
        status = "✅ OK" if fp_rate <= 30 else "⚠️ HIGH" if fp_rate <= 50 else "❌ FAIL"
        if total == 0:
            status = "— (no data)"
        print(
            f"  {det:<20} | {coin:<5} | {total:<8} | {correct:<8} | {wrong:<6} | "
            f"{accuracy:>7.0f}%   | {fp_rate:>7.0f}%   | {status}"
        )
...
for det in detectors:
    all_det = []
    for coin in ["BTC", "ETH", "SOL"]:
        all_det.extend([t for t in all_results[coin] if t["detector"] == det])
    total = len(all_det)
    correct = sum(1 for t in all_det if t["correct"])
    wrong = total - correct
    accuracy = correct / total * 100 if total > 0 else 0
    fp_rate = wrong / total * 100 if total > 0 else 0
    status = "✅ OK" if fp_rate <= 30 else "⚠️ HIGH" if fp_rate <= 50 else "❌ FAIL"
    print(
        f"  {det:<20} | {'ALL':<5} | {total:<8} | {correct:<8} | {wrong:<6} | "
        f"{accuracy:>7.0f}%   | {fp_rate:>7.0f}%   | {status}"
    )

# ══════════════════════════════════════════════════════════════════════
# SIDEWAYS vs VOLATILE BREAKDOWN
# ══════════════════════════════════════════════════════════════════════

print(f"\n\n{'=' * 90}")
print("SIDEWAYS (Apr 6) vs VOLATILE (Apr 7-8) BREAKDOWN")
print("=" * 90)

for period, label in [("Apr6", "SIDEWAYS"), ("Apr7", "PRE-PUMP"), ("Apr8", "PUMP")]:
    total = correct = 0
    for coin in ["BTC", "ETH", "SOL"]:
        period_triggers = [t for t in all_results[coin] if t["time"].startswith(period)]
        total += len(period_triggers)
        correct += sum(1 for t in period_triggers if t["correct"])
    wrong = total - correct
    fp = wrong / total * 100 if total > 0 else 0
    print(f"  {period} ({label:>10}): {total:>3} triggers | {correct} correct | {wrong} wrong | FP rate: {fp:.0f}%")
...
grand_correct = sum(sum(1 for t in v if t["correct"]) for v in all_results.values())
grand_fp = (grand_total - grand_correct) / grand_total * 100 if grand_total > 0 else 0

sideways_total = sideways_wrong = 0
for coin in ["BTC", "ETH", "SOL"]:
    sw = [t for t in all_results[coin] if t["time"].startswith("Apr6")]
    sideways_total += len(sw)
    sideways_wrong += sum(1 for t in sw if not t["correct"])
sideways_fp = sideways_wrong / sideways_total * 100 if sideways_total > 0 else 0

print(f"\n  Overall: {grand_total} triggers, {grand_correct} correct, FP rate: {grand_fp:.0f}%")
print(f"  Sideways (Apr 6): {sideways_total} triggers, FP rate: {sideways_fp:.0f}%")

if sideways_fp > 30:
    print(f"\n  ⚠️  SIDEWAYS FP RATE > 30% — threshold needs tuning")
elif grand_fp > 30:
    print(f"\n  ⚠️  OVERALL FP RATE > 30% — threshold needs tuning")
else:
    print(f"\n  ✅  ALL RATES WITHIN ACCEPTABLE RANGE — ready for production")
